...
const MODES: Array<{
  ...
  },
  {
    id: "theater-3d",
    label: "Theater 3D",
    description: "Range rings, flight tasking, spatial deconfliction",
    Icon: Route,
  },
  {
    id: "ops-wall",
...
export default function IntelligenceWorkspace() {
  ...
  const [extentMode, setExtentMode] = useState<"operational" | "selection" | "compare">("operational");
  ...
  const compareBBox = comparedIds
    .map((id) => features.find((feature) => feature.properties.id === id))
    .filter(Boolean)
    .reduce<BBox | undefined>((acc, feature) => mergeBBoxes(acc, computeFeatureBBox(feature as Feature)), undefined);
  ...
  }, [extentMode, selectedBBox, compareBBox, operationalBBox]);

  const dossierCards = useMemo(() => {
    return [...visibleFeatures].sort((a, b) => {
      const rank = { critical: 4, high: 3, medium: 2, low: 1 };
      return rank[b.properties.priority] - rank[a.properties.priority];
    });
  }, [visibleFeatures]);
  ...
  const incidentFeatures = visibleFeatures.filter((feature) => feature.properties.entityType === "incident" || feature.properties.entityType === "site");
  ...
  const renderEOOverlay = () => (
    <>
      ...
      {[
        ...
        <div key={index} className="absolute border-[3px] border-amber-400/95" style={{ left: `${x}%`, top: `${y}%`, width: `${w}%`, height: `${h}%` }} />
      ))}
      {[
        [40, 48, 12, 13],
        [44, 33, 12, 14],
        ...
        <div key={index} className="absolute border-[3px] border-blue-500/90" style={{ left: `${x}%`, top: `${y}%`, width: `${w}%`, height: `${h}%` }} />
      ))}
      ...
      {incidentFeatures.map((feature) => {
        const point = projectPoint(
          feature.geometry.type === "Point" ? feature.geometry.coordinates : selectionAnchor,
          activeBBox
        );
        return (
          <button
  ...
  const renderSite3D = () => (
    <>
      ...
      <div className="absolute left-5 top-4 rounded-md border border-[#b0bf63]/25 bg-black/55 px-4 py-3 text-sm text-zinc-100">
        ...
        <button className="ml-4 rounded-sm border border-[#b0bf63]/40 px-3 py-1 text-[#d6dd7b]">Task Asset</button>
      </div>
    </>
  );
  ...
  const renderTheater3D = () => (
    <>
      ...
      {[
        ...
        <div key={index} className="absolute text-sm font-medium text-zinc-100/78" style={{ left: `${x}%`, top: `${y}%` }}>{label}</div>
      ))}
    </>
  );
  ...
  const renderOpsWall = () => (
    <div className="absolute inset-0 bg-[#090909] p-6">
      ...
      <div className="grid h-[calc(100%-94px)] grid-cols-[260px_minmax(0,1fr)_280px] gap-4">
  ...
  return (
    <div className="min-h-screen bg-[#11120f] p-4 text-zinc-100 md:p-6">
      <div className="mx-auto max-w-[1620px] overflow-hidden rounded-[30px] border border-zinc-800 bg-[#161713] shadow-2xl">
        <div className="grid min-h-screen grid-cols-1 xl:grid-cols-[94px_360px_minmax(0,1fr)_400px]">
          <aside className="flex gap-3 border-r border-zinc-800 bg-[#12130f] p-3 xl:flex-col xl:gap-2">
          ...
          <aside className="border-r border-zinc-800 bg-[#181914] p-5">
            <div>
              ...
              <div className="mt-1 text-sm text-zinc-400">Expanded screen-family preview: 2D map, EO overlay, site 3D, theater 3D, ops wall</div>
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-zinc-800 bg-[#13140f] p-3">
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-zinc-200"><Monitor className="h-4 w-4 text-[#d1b16d]" /> Screen Families</div>
                <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Missing coverage closed</div>
              </div>
